...
* 영화 및 TV 프로그램에 대한 키워드 기반 검색 기능
* Swiper를 이용한 슬라이드형 UI 구성
* 상세 페이지에서 줄거리, 출연진, 영상, 추천 콘텐츠 제공
* 반응형 UI
...
### 1. TheMovieDB API 연동 및 비동기 데이터 처리
* TheMovieDB 오픈 API를 활용하여 영화 및 TV 프로그램의 다양한 실시간 정보를 동적으로 불러와 제공
* Axios (또는 fetch API)를 이용한 비동기 데이터 요청 및 응답 처리 
* 데이터 로딩 상태(로딩 중, 성공, 에러)를 관리하여 사용자에게 시각적인 피드백 제공 (예: 로딩 스피너)

### 2. 키워드 기반 검색
* 사용자가 검색어를 입력 후 'Search' 버튼 클릭 시 해당 키워드를 기반으로 미디어 검색 결과 제공
* 검색 결과를 실시간으로 화면에 업데이트하여 빠른 탐색 경험 제공
* 검색 결과가 없을 경우 NoData 컴포넌트를 통해 사용자에게 명확한 정보 전달

### 3. 미디어 상세 정보 제공
* 각 미디어(영화/TV 프로그램) 클릭 시 React Router를 통해 동적 라우팅된 상세 페이지로 이동
* 상세 페이지에서 포스터, 개봉일, 줄거리, 출연진, 영상 등 풍부한 정보 표시
* 유튜브 oEmbed API를 활용하여 유효한 영상만 필터링하고 최대 3개까지 표시하는 로직 구현
* 장르 기반의 '비슷한 콘텐츠 추천' 기능을 구현하여 사용자에게 맞춤형 콘텐츠 탐색 기회 제공

### 4. Swiper Slide UI 및 '더보기' 기능
* Swiper 라이브러리를 활용하여 홈 페이지의 메인 배너 및 카테고리별 미디어 리스트를 슬라이드 형태로 구성
* 모바일 환경에서도 자연스러운 터치 기반 UX를 지원하도록 구현
* 긴 목록의 경우 '더보기' 버튼을 통해 점진적으로 데이터를 로딩하여 초기 로딩 속도 및 성능 최적화

### 5. 효율적인 상태 관리
* Zustand를 이용하여 API를 통해 가져온 데이터 및 UI 관련 전역 상태를 효율적으로 관리하고 컴포넌트 간 데이터 흐름을 최적화
* 페이지 이동 시 **window.scrollTo(0,0)**을 통해 스크롤 위치를 최상단으로 초기화하여 사용자 경험 개선
...
### 1. 페이지 스크롤 위치 문제 해결
- **상황**: 페이지 이동 시 스크롤이 내려간 상태로 로드되거나 로딩 화면이 잘려 보임
...
### 2. 유효하지 않은 영상 데이터 처리
- **상황**: 상세 페이지에 영상이 표시되지 않거나, 썸네일이 깨지고 클릭해도 영상이 존재하지 않는 문제 발생
...
### 3. 허전한 레이아웃 변경
- **상황**: TV 프로그램의 시즌 정보를 Swiper로 보여주던 중, `overview` 등의 정보가 부족한 경우 내용이 매우 짧고 포스터는 세로형이라 빈 부분이 많이 생기는 문제 발생
